...
const startIdx = lines.findIndex(l => l.includes('setIsProcessing(false);'));
if (startIdx !== -1) {
  // The '  };' is 2 lines below startIdx
  const endIndex = lines.findIndex((l, i) => i > startIdx && l.includes('function OverviewManager() {'));
  ...
  if (endIndex !== -1) {
    ...
    console.log('Fixed syntax error via lines!');
  }
}
